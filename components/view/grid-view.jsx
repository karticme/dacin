    ...
    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4 p-4 py-6">
      {data.map((item) => (
        ...
        >
          ...
          >
            <div className="w-3/5 aspect-square p-1 rounded-sm group-data-selected/item:bg-muted group-data-pressed/item:bg-muted overflow-hidden transition-colors ease-in-out duration-600">
              <img
                src={item.thumbnail}
                alt={item.name}
            ...
            <div className="relative flex flex-col pb-2">
              <Truncated
                value={item.name}
                lines={2}
                className="text-muted-foreground text-[13px] text-center group-data-selected/item:text-info group-data-pressed/item:text-info transition-colors ease-in-out"
              />
              {item.size && (
