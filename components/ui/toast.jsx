...
function Toasts({ position, portalProps }) {
  ...
  return (
    <Toast.Portal data-slot="toast-portal" {...portalProps}>
      <Toast.Viewport
        className={cn(
          "fixed z-60 mx-auto flex w-[calc(100%-var(--toast-inset)*2)] max-w-90 [--toast-inset:--spacing(4)] sm:[--toast-inset:--spacing(6)]",
          ...
          return (
            <Toast.Root
              ...
              className={cn(
                ...
                "[--toast-calc-height:var(--toast-frontmost-height,var(--toast-height))] [--toast-gap:--spacing(2.5)] [--toast-peek:--spacing(2)] [--toast-scale:calc(max(0,1-(var(--toast-index)*.1)))] [--toast-shrink:calc(1-var(--toast-scale))]",
...
function AnchoredToasts({ portalProps }) {
  const { toasts } = Toast.useToastManager();
...
export function AnchoredToastProvider({ children, portalProps, ...props }) {
  return (
